...
<form id="identity-form" on:submit|preventDefault={submit}>
    <fieldset class="mb-3 bg-white shadow-lg rounded text-gray-600">
        ...
        <div class="my-4 xl:mx-4 flex flex-col xl:flex-row justify-center items-center">
            <button disabled={!isValid} type="submit" class="text-center w-1/2 px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-75">{$t("auth.register")}</button>
