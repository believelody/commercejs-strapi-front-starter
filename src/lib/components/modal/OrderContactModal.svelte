<script>
    ...
    import ContactForm from "../form/ContactForm.svelte";
...
<WithActionModal>
    <span class="text-gray-600" slot="header">{$t("order.modal.contact.title", { reference })}</span>
    <div slot="content">
        <h4 class="mx-2">
            <span class="text-gray-600">{$t("identity.title")} : </span>
            <span class="text-gray-600">{customer.firstname} {customer.lastname}</span>
        </h4>
        <h4 class="mx-2">
            <span class="text-gray-600">{$t("identity.email.label")} : </span>
